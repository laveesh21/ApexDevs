    ...
    return (
      <div className="min-h-screen bg-dark-900 flex flex-col items-center justify-center px-6">
        <div className="text-center">
          ...
          <Link to="/" className="inline-block px-6 py-3 bg-primary hover:bg-primary-light text-dark-900 font-medium rounded-lg transition-colors">Back to Home</Link>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-dark-600">
          ...
          {isOwner && (
            <div className="flex gap-2">
              <button className="flex items-center gap-2 px-4 py-2 bg-dark-700 hover:bg-dark-600 border border-dark-600 hover:border-primary/50 text-gray-300 rounded-lg transition-all" onClick={() => setShowEditForm(true)} title="Edit Project">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <section className="bg-dark-800 border border-dark-600 rounded-xl p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {project.technologies?.map((tech, index) => (
                  <span key={index} className="px-3 py-1.5 bg-primary/20 border border-primary/30 text-primary rounded-md text-sm font-medium">{tech}</span>
                ))}
              </div>
            </section>
            ...
            <section className="bg-dark-800 border border-dark-600 rounded-xl p-6">
                ...
                <div className="mb-6">
                  {userReview ? (
                    <div className="bg-dark-700 border border-dark-600 rounded-lg p-4">
                      ...
                      <div className="flex gap-2">
                        <button className="px-4 py-2 bg-dark-600 hover:bg-dark-500 border border-dark-500 text-gray-300 rounded-lg transition-colors text-sm" onClick={() => setShowReviewForm(true)}>
                          Edit Review
                        </button>
              ...
              {showReviewForm && (
                <div className="bg-dark-700 border border-dark-600 rounded-lg p-4 mb-6">
                    ...
                    <button
                      className={`px-4 py-2 rounded-lg font-medium transition-all ${
                        reviewFormData.rating === 'like'
                          ...
                          : 'bg-dark-600 border border-dark-500 text-gray-400 hover:border-green-500/50'
                      }`}
                    ...
                    <button
                      className={`px-4 py-2 rounded-lg font-medium transition-all ${
                        reviewFormData.rating === 'dislike'
                          ...
                          : 'bg-dark-600 border border-dark-500 text-gray-400 hover:border-red-500/50'
                      }`}
                  ...
                  <textarea
                    className="w-full px-4 py-2 bg-dark-600 border border-dark-500 text-gray-100 rounded-lg focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary mb-4"
                  ...
                  <div className="flex justify-end gap-2">
                    <button className="px-4 py-2 bg-dark-600 hover:bg-dark-500 border border-dark-500 text-gray-300 rounded-lg transition-colors" onClick={() => {
                      setShowReviewForm(false);
                      setReviewFormData({ rating: '', comment: '' });
                    }}>
                      Cancel
              ...
              {reviews.length > 0 && (
                <div>
                  ...
                  <div className="space-y-4">
                    {reviews.map((review) => (
                      <div key={review._id} className="bg-dark-700 border border-dark-600 rounded-lg p-4">
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-dark-600">
          ...
          <div className="space-y-6">
            {/* Project Info Card */}
            <div className="bg-dark-800 border border-dark-600 rounded-xl p-6 sticky top-8">
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <div>
                    <Link to={`/user/${project.author?._id || project.author?.id}`} className="block text-gray-100 font-medium hover:text-primary transition-colors">
                      {project.author?.username || 'Unknown'}
                    </Link>
                    <span className="text-sm text-gray-500">{project.author?.role || 'Developer'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                {!project.demoUrl && !project.githubUrl ? (
                  ...
                  <div className="space-y-2">
                    {project.demoUrl && (
                      <a href={project.demoUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-4 py-3 bg-dark-700 hover:bg-dark-600 border border-dark-600 hover:border-primary/50 rounded-lg transition-all group">
                        <span className="text-2xl">🌐</span>
                        <span className="text-gray-300 group-hover:text-primary font-medium">Live Demo</span>
                      </a>
                    )}
                    {project.githubUrl && (
                      <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-4 py-3 bg-dark-700 hover:bg-dark-600 border border-dark-600 hover:border-primary/50 rounded-lg transition-all group">
                  ...
                  <button
                    className={`w-full py-3 rounded-lg font-medium transition-all ${
                      isLiked
                        ...
                        : 'bg-dark-700 border border-dark-600 text-gray-300 hover:border-primary/50 hover:bg-dark-600'
                    }`}
                    onClick={async () => {
                      if (!token) {
